...
void main(void) {
	...
	vec3 diffuse_component = (max(0.0, dot(normal_vector, normalize(light_direction))) * light_color) / ((1.01 - diffuse_strength) * length(light_direction));
	...
	float specular_factor = pow(max(0.0, dot(reflection_direction, view_direction)), 128);
