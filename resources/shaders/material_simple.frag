...
void main(void) {
	if (quantity_directional_lights == 0 && quantity_point_lights == 0 && quantity_spot_lights == 0) { fragment_color = vec4(material.color_diffuse, 1.0); return; }
	...
	for (unsigned int i = 0; i < quantity_directional_lights; ++i) {
		...
		vec3 diffuse_component = light.color_diffuse * material.color_diffuse;

		float specular_factor = pow(max(0.0, dot(light.direction, -view_direction)), material.gloss_factor);
		vec3 specular_component = light.color_specular * material.color_specular * specular_factor;
